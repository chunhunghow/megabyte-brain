...
class Attention(nn.Module):
    ...
    def forward(self, x, memory=None,attn_bias = None):
        ...
        if memory is None:
            ...
            q, k, v = (self.to_q(x), *self.to_kv(memory).chunk(2, dim = -1))
...
class TransformerDecoder(nn.Module):
        ...
        for i in range(layers):
            if cross:
                self.layers.append(nn.ModuleList([
                    ...
                    Attention(dim = dim, dim_head = dim_head, heads = heads, dropout = attn_dropout, flash = flash_attn, causal=False),
    ...
    def forward(self, x, memory=None):
        ...
        for s_attn, c_attn, ff in self.layers:
            x = s_attn(x, attn_bias = attn_bias) + x
            if self.cross:
                assert memory is not None
                x = c_attn(x, memory, attn_bias = None) + x
            x = ff(x) + x
...
class MEGABYTE_Decoder(nn.Module):
    def __init__(
        self,
        *,
        num_tokens,
        ...
        for i, stage_depth in enumerate(depth):
            self.transformers.append(TransformerDecoder(
                dim = dim,
                layers = stage_depth,
                dim_head = dim_head,
                heads = heads,
    ...
    def original_generate(self, memory, prime = None, filter_thres = 0.9, temperature = 1., default_batch_size = 1):
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(range(total_seq_len - seq.shape[-1])):
                logits = self.forward(seq, memory)[:, -1]
    ...
    def generate(self, memory, prime = None, filter_thres = 0.9, temperature = 1., default_batch_size = 1024):
        #total_seq_len = reduce_mult(self.max_seq_len)
        total_seq_len = self.max_seq_len[-1] # the local
        device = next(self.parameters()).device

        #if not exists(prime):
        if prime.shape[-1] ==1:
            #prime = torch.empty((default_batch_size, 0), dtype = torch.long, device = device)
            prime = torch.cat([prime, torch.zeros_like(prime).repeat(1,default_batch_size-1).to(prime.device)],-1)
            prime = prime.reshape(-1,1)
        seq = prime
        batch = seq.shape[0]
        out_logits = []
        with torch.no_grad():
            #for _ in tqdm(range(total_seq_len - seq.shape[-1])):
            for _ in tqdm(range(total_seq_len )):
                logits = self.forward(seq, memory)[:, -1]
                out_logits += [logits[:,None,:].cpu()] 
        ...
        #seq = rearrange(seq, "(b k) p -> b k p", k=memory[1].shape[1])
        #seq = rearrange(seq, "b k p -> b (k p)")
        #return seq

        logits = torch.cat(out_logits, -2)
        logits = rearrange(logits,"(b k) p d -> b k p d" , b = memory[0].shape[0])
        logits = rearrange(logits, 'b ... n -> b (...) n')

        return logits

    def forward_empty(self, batch_size, memory):
        # take care of special case
        # where you sample from input of 0 (start token only)
        tokens = repeat(self.start_tokens, 'd -> b 1 d', b = batch_size)
        for i, transformer in enumerate(self.transformers):
            if i == 0:
                #tokens = rearrange(tokens, "(b k) 1 d -> b k d", b=memory[i].shape[0])
                #tokens = transformer(tokens, memory[i])
                pass
            else:
                tokens = transformer(tokens, rearrange(memory[i], "b k ... -> (b k) ..."))

        return self.to_logits(tokens)

    def forward(self, ids, memory, return_loss = False):
        batch = ids.shape[0]
        ...
        if ids.numel() == 0:
            return self.forward_empty(memory[1].shape[0] * memory[1].shape[1], memory)
        ...
        # we replace the start token with task ID token, first in the sequence
        ...
        for ind, (stage_tokens, transformer) in enumerate(zip(tokens_at_stages, self.transformers)):
            ...
            if ind == 0:
                ...
                attended = transformer(stage_tokens, rearrange(memory[ind], "b k ... -> (b k) ..."))

            attended = unpack_one(attended, ps, '* n d')

            # dropping the last one, and force the all previous to match with original stage token before concat
            # predict next token
