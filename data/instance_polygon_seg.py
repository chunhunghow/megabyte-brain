    ...
    def __getitem__(self, idx):
        ...
        if (self.mode == 'abnormal') & self.label:
            ...
            if self.train:
                ...
                #im = augmented['image'].repeat(3,1,1)
                im = augmented['image']
                mask = augmented['mask']
            else:
            ...
            if self.train:
                return im, mask, boxes, contours_list
            else:
                return im[None,...], mask, boxes, contours_list
...
@dataset_lib.DatasetRegistry.register('polygon_lesion_segmentation')
class InstanceSegDataset(dataset_lib.Dataset):
    ...
    def collate_fn(batch):
        ...
        return im, mask, torch.cat(boxes), polygons
    ...
    def process(self, batch, target_format='xyxyn'):
        """
        ...
        if target_format == 'xyxy':
            ...
            polygons = polygons.float()
            polygons[0::2] /= im.shape[-1]
            polygons[1::2] /= im.shape[-2]
        ...
        return im, mask.cpu() ,boxes,polygons
